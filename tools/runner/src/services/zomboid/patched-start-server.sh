...
# Patched by Pommaq's memtool to propagate signals properly
############
...
if "${INSTDIR}/jre64/bin/java" -version > /dev/null 2>&1; then
	...
	JSIG="libjsig.so"
	LD_PRELOAD="${LD_PRELOAD}:${JSIG}" exec ./ProjectZomboid64 "$@"
	...
	JSIG="libjsig.so"
	LD_PRELOAD="${LD_PRELOAD}:${JSIG}" exec ./ProjectZomboid32 "$@"
else
